...
import { OrbitControls, Environment, Stars, Image, Float, Html } from '@react-three/drei';
...
// Safe generation of mock photos
    ...
    if (typeof document !== 'undefined') {
        try {
            ...
            if (ctx) {
                ...
                grad.addColorStop(1, `hsl(${(hue + 40) % 360}, 70%, 60%)`);
                ctx.fillStyle = grad;
                ...
                ctx.fillStyle = 'white';
                // Fallback font stack
                ctx.font = 'bold 60px "Ma Shan Zheng", "Arial", sans-serif'; 
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.shadowColor = 'rgba(0,0,0,0.2)';
                ctx.shadowBlur = 10;
                ctx.fillText(`美好回忆 ${index + 1}`, 200, 150);
            }
            url = canvas.toDataURL('image/jpeg', 0.6); // Lower quality for speed
        } catch (e) {
            console.error("Failed to generate mock photo", e);
            url = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'; // transparent placeholder
        }
    }
    return { id: `p-${index}`, url };
};
...
const GestureLegend: React.FC<{ activeGesture: GestureType; visible: boolean }> = ({ activeGesture, visible }) => {
    ...
    return (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-30 flex gap-6 animate-in fade-in slide-in-from-top-4 duration-700">
             {items.map((item) => {
                 ...
                 return (
                    <div 
                        key={item.type} 
                        className={`
                            flex items-center justify-center w-14 h-14 rounded-full border backdrop-blur-md transition-all duration-300 select-none
                            ${isActive 
                                ? 'bg-white/20 border-white/50 scale-110 shadow-[0_0_20px_rgba(255,255,255,0.3)]' 
                                : 'bg-black/20 border-white/10 hover:bg-white/10 opacity-70'}
                        `}
                    >
                        ...
                        {isActive && (
                            <div className="absolute -bottom-1 -right-1 w-2.5 h-2.5 rounded-full bg-green-400 shadow-[0_0_8px_#4ade80]" />
                        )}
                    </div>
                 );
             })}
...
const PolaroidFocus: React.FC<PolaroidFocusProps> = ({ photo, onClose, onReplace }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            onReplace(photo.id, e.target.files[0]);
        }
    };
    ...
    return (
        <>
            ...
            <Float speed={2} rotationIntensity={0.05} floatIntensity={0.05}>
                <group 
                    ref={groupRef}
                    // Click on the photo triggers file replacement
                    onClick={(e) => {
                        e.stopPropagation();
...
// -- Gesture Manager Component (Inside Canvas) --
const GestureManager: React.FC<{
    ...
    // Throttle MediaPipe processing to save resources
    useFrame(({ clock }) => {
        if (!isCameraReady || !videoRef.current) return;
        
        // Process every ~100ms (10fps) is enough for gestures
        const t = clock.elapsedTime;
        if (Math.floor(t * 10) % 2 !== 0) return; 
...
const App: React.FC = () => {
  ...
  // Load photos safely on mount
  useEffect(() => {
    const loadPhotos = async () => {
        // Generate in small batches or just one go but slightly delayed to let UI render first
  ...
  const handleDoubleClick = useCallback(() => {
    if (mode === 'tree') {
      // Cycle: Green(0) -> Gold(1) -> Silver(2) -> Rainbow(3) -> Green(0)
  ...
  const handleGesture = useCallback((gesture: GestureType) => {
      ...
      if (gesture === 'Open_Palm') {
          ...
          // Only collapse if we were previously holding palm and now stopped
          // This allows mouse scroll to still work without fighting gesture 'None'
          setIsExploded(false);
      }

      // 2. Closed Fist -> Twinkle
      ...
      // 3. Victory -> Rapid Color Change
      if (gesture === 'Victory') {
          const now = Date.now();
          // Throttle to 200ms to avoid seizure-inducing strobing
          if (now - lastColorChange.current > 200) {
              setTreeColorIndex(prev => (prev + 1) % TREE_COLORS.length);
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const newPhotos: PhotoData[] = Array.from(e.target.files).map((file: File) => ({
        id: file.name + Date.now() + Math.random(), // Ensure unique ID
  ...
  const handleReplacePhoto = (id: string, file: File) => {
      ...
      // Update active photo immediately so visual changes
      if (activePhoto && activePhoto.id === id) {
          setActivePhoto({ ...activePhoto, url: newUrl });
      }
  };
  ...
  const generateVideo = useCallback(() => {
    ...
    setIsTwinkling(false);
    setGestureX(0);
    setTreeColorIndex(0); // Reset color to green
    ...
    // Step 4: Double Click -> Change Colors (Gold -> Silver -> Rainbow)
    setTimeout(() => setGuideType('doubleClick'), 22000); // Show Guide
    setTimeout(() => {
        setGuideType(null);
        setTreeColorIndex(1); // Gold
    }, 24500);
    
    setTimeout(() => setTreeColorIndex(2), 25500); // Silver
    setTimeout(() => setTreeColorIndex(3), 26500); // Rainbow
    
    // Step 5: Album -> Enter Album -> Auto Scroll
    setTimeout(() => {
        setMode('album');
    }, 28000);
    ...
    // Greeting & End
    setTimeout(() => setShowGreeting(true), 33500);
    setTimeout(() => recorder.stop(), 37500);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Gesture Loop hooked into Canvas Frame */}
        ...
        >
            ...
            >
                ...
                <div className="text-center">
                    ...
                    {/* Fixed Static QR Code */}
                    <div className="relative mx-auto w-48 h-48 bg-white rounded-2xl p-2 shadow-lg mb-6 overflow-hidden">
                        <img 
                            src="/payment_qr.jpg" 
                            alt="Payment QR" 
                        ...
                        <div className="fallback hidden w-full h-full border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center bg-gray-50 text-gray-400 flex-col gap-2 absolute top-2 left-2 right-2 bottom-2">
                            <span className="text-4xl opacity-50">📷</span>
                            <span className="text-xs text-center px-1">请将收款码命名为 payment_qr.jpg 并放入 public 文件夹</span>
                        </div>
                    </div>
